...
def batch_train(FLAGS):
    ...
    df_data = pd.read_csv(os.path.join(FLAGS.data_dir, 'data.txt'), usecols=[
                          'image_id', 'steering_angle'])
    ...
    net_arch.autopilot_net(features)
    ...
    for epoch in range(num_epochs):
        ...
        for _ in range(num_batches_train):
            _, loss_per_batch = ss.run([optimizer_op, loss], feed_dict={
                                       training_pl: True, global_step: epoch})
        ...
        print('training loss : {0:.4f}, validation loss : {1:.4f}'.format(
            temp_train_loss_per_epoch / num_batches_train, temp_valid_loss_per_epoch / num_batches_valid))
        print('')
    ...
    np.save(os.path.join(os.getcwd(), model_dir,
                         FLAGS.model_metrics), np.array(losses_dict))
