...
class AutoPilot:
    ...
    def autopilot_net(self, features):
        ...
        self.dense_relu1 = self._get_relu_activation(
            self.dense1, name='dense_relu1')
        ...
        self.dense_relu2 = self._get_relu_activation(
            self.dense2, name='dense_relu2')
        ...
        self.dense_relu3 = self._get_relu_activation(
            self.dense3, name='dense_relu3')
        ...
        self.dense_relu4 = self._get_relu_activation(
            self.dense4, name='dense_relu4')
    ...
    def _get_conv2d_layer(self, input_layer, num_filters, kernel_size, strides, padding, name='conv'):
        return tf.layers.conv2d(inputs=input_layer, filters=num_filters, kernel_size=kernel_size, strides=strides, padding=padding, data_format=self._data_format, kernel_initializer=self._initializer, name=name)
    ...
    def _get_dropout_layer(self, input_layer, rate=0.5, name='dropout'):
        return tf.layers.dropout(inputs=input_layer, rate=rate, training=self._training, name=name)

    # dense layer
    def _get_dense_layer(self, input_layer, num_neurons, use_bias=True, name='dense'):
        return tf.layers.dense(input_layer, units=num_neurons, use_bias=use_bias, name=name)
