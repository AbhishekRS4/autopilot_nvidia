...
def infer(FLAGS):
    ...
    df_data = pd.read_csv(os.path.join(
        FLAGS.data_dir, 'data.txt'), index_col=0)
    df_data_dict = df_data.to_dict('index')

    outputs_dir = 'outputs_' + FLAGS.which_set + \
    ...
    images_list_infer = np.load(os.path.join(
        os.getcwd(), model_dir, 'data_dict.npy')).item()[FLAGS.which_set + '_list']
    ...
    net_arch = autopilot_model.AutoPilot(training_pl, FLAGS.data_format)
    net_arch.autopilot_net(image_pl)
    ...
    for img_id in images_list_infer:
        img = cv2.cvtColor(cv2.imread(os.path.join(
            images_dir_infer, img_id)), cv2.COLOR_BGR2RGB)
        img_pre = img.astype(np.float32) / 255.
        img_pre = img_pre.reshape(
            1, img_pre.shape[0], img_pre.shape[1], img_pre.shape[2])
        img_pre = np.transpose(img_pre, [0, 3, 1, 2])

        ti = time.time()
        theta_pred = ss.run(logits, feed_dict={
                            image_pl: img_pre, training_pl: False})
        ti = time.time() - ti
    ...
    print('Predictions saved in : ' + str(os.path.join(model_dir, outputs_dir)))
    print('')
    ss.close()
